...
*/
		...
		chloralhydrate
			...
			result_amount = 1
			on_reaction(var/datum/reagents/holder, var/created_volume, mob/user as mob)
				log_chem("[key_name(usr)] made Chloralhydrate.")
				message_admins("\blue [key_name(usr)] made Chloralhydrate.(<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[usr.x];Y=[usr.y];Z=[usr.z]'>JMP</a>)")
				return

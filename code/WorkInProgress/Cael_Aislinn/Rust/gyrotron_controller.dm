...
/obj/machinery/computer/rust_gyrotron_controller
	...
	var/updating = 1
	var/id_tag = 1
	var/list/connected_gyros = list()

	New()
		..()
		for(var/obj/machinery/rust/gyrotron/gyro in world)
			if(gyro.id_tag == src.id_tag && !gyro.connected_console)	//If the id_tags are the same and the gyro isn't already connected to a console, connect it
				connected_gyros += gyro
				gyro.connected_console = src
	...
	interact(mob/user)
	//	world << "gyrotron_control: Break 1"
		if ( (get_dist(src, user) > 1 ) || (stat & (BROKEN|NOPOWER)) )
		//	world << "gyrotron_control: Break 2"
			if (!istype(user, /mob/living/silicon))
		//		world << "gyrotron_control: Break 3"
				user.machine = null
				user << browse(null, "window=gyrotron_controller")
				return
		var/t = "<B>Gyrotron Remote Control Console</B><BR>"
		t += "<hr>"
		for(var/obj/machinery/rust/gyrotron/gyro in connected_gyros)
	//		if(gyro.id_tag == src.id_tag && !gyro.connected_console)	//If the id_tags are the same and the gyro isn't already connected to a console, connect it
	//			connected_gyros += gyro									//Moved up to New()

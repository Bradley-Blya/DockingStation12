...
/obj/machinery/rust/gyrotron
	...
	var/remoteenabled = 1
	var/id_tag = 1
	var/obj/machinery/computer/rust_gyrotron_controller/connected_console = null
	//
	...
	Topic(href, href_list)
		...
		if( href_list["modifypower"] )
			...
			if(connected_console)
				connected_console.updateDialog()
			return
...
//				comp.updateUsrDialog()
			if(connected_console)
				connected_console.updateUsrDialog()
			return
...
*/
	proc/Emit()
		...
		A.damage = mega_energy * 1000
		//
		A.icon_state = "emitter"
		playsound(src.loc, 'sound/weapons/emitter.ogg', 25, 1)
		use_power(100 * mega_energy + 1000)
		/*if(prob(35))
			var/datum/effect/effect/system/spark_spread/s = new /datum/effect/effect/system/spark_spread
			s.set_up(5, 1, src)
			s.start()*/
